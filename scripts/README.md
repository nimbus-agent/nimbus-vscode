...
| `run-ui-tests.mjs` | `bun run test:ui` | Build the extension, compile the UI specs, start the fake Gateway (`test/ui/fake-gateway.ts`), and run the ExTester/Selenium suite against it in a real VS Code — see [`docs/development.md`](../docs/development.md#ui-tests). CI runs it too (Linux/xvfb only). |
