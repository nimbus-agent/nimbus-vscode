...
| --- | --- | --- |
| `clean.mjs` | `bun run clean` | Remove build artifacts (`dist/`, `media/webview.*`, `media/context.*`) and the UI-test harness output (`out/`, `test-resources/`). It lists targets one by one, so every new `esbuild.mjs` output needs a line — one that is missing survives a clean. |
| `check-bundle.mjs` | `bun run check-bundle` | Assert two of the three bundles (`dist/extension.js`, `media/webview.js` — `media/context.js` is covered by `check-vsix-contents.mjs` instead) exist and the extension bundle keeps `vscode` as its only external require — i.e. `@nimbus-dev/client` and all other deps are inlined, so the `.vsix` has no runtime npm dependency. Run after `bun run build`; CI runs it too. |
...
| `check-settings-docs.mjs` | `bun run check-settings-docs` | Assert every `nimbus.*` configuration setting in `package.json` is documented in `docs/settings.md` and the README settings table — and, in the other direction, that neither documents a setting `package.json` no longer contributes, so a rename cannot leave a stale half behind. Presence only: defaults, ranges and prose are unchecked. CI runs it too. |
| `run-ui-tests.mjs` | `bun run test:ui` | Build the extension, compile the UI specs, start the fake Gateway (`test/ui/fake-gateway.ts`), and run the ExTester/Selenium suite against it in a real VS Code — see [`docs/development.md`](../docs/development.md#ui-tests). Local-only — CI does not run this suite (see [`docs/development.md`](../docs/development.md#ui-tests) for the upstream headless-Linux limitation). |
