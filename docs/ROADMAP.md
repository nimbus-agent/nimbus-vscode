...
| **Sidebar** — Audit, Sessions (with chat resume), Index, Agents, Workflows | `auditList`, `getSessionTranscript`, `queryItems`, `workflowList`, `workflowListRuns` |
| **Workflows view** — read-only: every saved workflow, with its recent runs (status, duration, trigger, dry-run badge, error) loaded on expand. **Monitor only** — running and cancelling are not shipped; see Phase 2 | `workflowList`, `workflowListRuns` |
...
| --- | --- | --- | --- |
| **Workflow surface** — ~~monitor~~ / run / cancel workflows | The flagship gap (the removed `Run Workflow` stub is tracked here). **Monitor shipped** as the read-only Workflows view — see *Already shipped*. **Cancel was never a client gap**: no `workflow.cancel` existed in the Gateway at all, and untagged `agent.chunk` made concurrent runs indistinguishable. Both are fixed in Nimbus#1165, typed in nimbus-client#60; the run surface waits on that client release | `workflowRun`, `workflowRunStream`, `workflowCancel` (unreleased), `workflowSave` / `workflowDelete` (authoring deferred: `steps_json` is opaque at save time, so a malformed DAG saves cleanly and fails only at run time) | M |
