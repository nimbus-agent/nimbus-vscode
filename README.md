...
- **Dev-workflow trio** — *Generate Commit Message* drafts a message from your staged diff, in your repository's own commit style, into the Source Control input box; *Review Changes* reviews all local changes (staged and unstaged) in a findings tab that also names what wasn't reviewed (too large, possibly secret, untracked); *Generate Tests* and *Generate Docstrings* work over an editor selection, opening an untitled test buffer or a docstring diff. Output is always a suggestion — nothing is written to disk or applied automatically.
