...
### Task 4: Bump `@nimbus-dev/client` to `^0.5.0`

**Independent of the feature** — it rides no preset code and the feature rides no new client capability. Sequenced last so its `package.json` edit does not collide with Task 2 (which also edits `package.json`), and so the bump lands as its own clean, revertable commit.

**Context:** `@nimbus-dev/client` was just extracted to its own repo (`nimbus-agent/nimbus-client`) and published its first standalone release, `@nimbus-dev/client@0.5.0`, to npm. This repo currently pins `^0.4.0` and its `bun.lock` resolves `0.4.0`; move it to `0.5.0`. `0.5.0`'s source is **identical** to `0.4.0` (no API change) — a safe bump; the build is expected green.

**Files:**
- Modify: `package.json` (the `@nimbus-dev/client` entry under `dependencies`)
- Modify: `bun.lock` (regenerated by `bun install`)

**Interfaces:** none — dependency/lockfile change only, no source or test changes.

- [ ] **Step 1: Change the pinned version**
...
Gotcha: if `bun.lock` still resolves `0.4.0`, bun's cached npm metadata (packument) is stale — run `bun pm cache rm` first, then `bun install` (or force the exact version with `bun add @nimbus-dev/client@0.5.0`).

- [ ] **Step 3: Confirm the resolved versions**

Confirm `bun.lock` now resolves `@nimbus-dev/client@0.5.0` (and its transitive `@nimbus-dev/sdk` moves to `^1.3.0`).

Run: `grep -n "@nimbus-dev/client\|@nimbus-dev/sdk" bun.lock`
Expected: the `@nimbus-dev/client` entry shows `0.5.0`; `@nimbus-dev/sdk` shows a `1.3.x` resolution.

- [ ] **Step 4: Verify no breakage**

Run: `bun run typecheck && bun run build && bun run test && bun run check-bundle`
Expected: all PASS (`0.5.0` is API-identical to `0.4.0`; the bundling invariant — `vscode` as the only external — is unchanged, so `check-bundle` stays green).

- [ ] **Step 5: Commit**

```bash
git add package.json bun.lock
git commit -m "chore: bump @nimbus-dev/client to ^0.5.0
