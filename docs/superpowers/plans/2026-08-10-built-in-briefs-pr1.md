...
- ~~**One stated assumption, deliberately not resolvable here:**~~ **RESOLVED 2026-08-10 — the Gateway is 1-based, so `toOneBased` is correct.** Settled by probing `agentsWhyPeek` (which shares `WhyParams`) against `ops-commands.ts`, whose 1-based line 2 is uniquely commit `caec0e0` while lines 1 and 3 are `475d24b`: querying line 2 returned `caec0e0` and `subject.lineNo` echoed 2. End-to-end, an editor on gutter line 2 (`active.line === 1`) renders `Why \`src/chat-participant/ops-commands.ts:2\`` with that line's real authorship. Passing VS Code's 0-based value straight through would have made every `why` answer about the line above.
  - Prerequisite worth recording for PR 2: this needs `nimbus init` to have indexed the repo root. Without it the Gateway returns *"Cannot anchor authorship: no resolvable file/line subject for this ref"* and every `whyPeek` field is null — so PR 2's hover is untestable until the root is indexed and the Gateway restarted to load it.
