...
| **Ambient context panel** — offer the agents that fit what is already on screen (file, selection, diff vs `HEAD`, branch) instead of waiting for a prompt; the editor counterpart of the browser ambient panel. **First slice shipped** (`nimbus.contextView`): diagnostics, branch and catalog-derived offers, on local reads alone — no RPC yet. Still open: blame (`agentsWhyPeek`) and related (`searchRanked`), the cache/coalescing/invalidation half of the cadence, and a real-editor pass | The surface with the richest context stops making you retype the context | `searchRanked` / `queryItems` + the `agents*` family | L |
