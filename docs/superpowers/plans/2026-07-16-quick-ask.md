...
Add the progress-location constant (top of the file, near the `Thenable` alias) so the handler avoids a magic number:
...
Add the imports (with the other module imports near the top). `PROGRESS_LOCATION_NOTIFICATION` is a value import from the shim (extension.ts already imports shim *types*; add a separate value import):

```ts
import { QUICK_ASK_MAX_CONTEXT_CHARS, buildQuickAskPrompt, clampContext, extractReply, validateQuestion } from "./quick-ask.js";
...
- Spec §"Components → quick-ask.ts" → Task 1 (`clampContext`, `buildQuickAskPrompt`, `extractReply`, `validateQuestion`, cap constant).
- Plan-review #3 (whitespace-only selection → whole-file fallback) → Task 2 handler (`selectionText.trim()` gate) + its dedicated test.
