...
1. **Trigger: passive + command.** Contribute the walkthrough so VS Code surfaces it on the Welcome / "Get Started" page for fresh installs, plus a `Nimbus: Open Walkthrough` command to open it anytime. **No auto-popup, no `globalState`, no shim change** — deliberately respecting users who dislike first-run popups and keeping the code surface tiny.
...
| 6 | `explore` | Explore Nimbus | `explore.md` | `command:nimbus.troubleshootConnection` | `onCommand:nimbus.troubleshootConnection` |
...
- Step 6 (`explore`) copy points at the sidebar views (Audit / Sessions / Index / Agents), the egress status-bar badge, and `Nimbus: Troubleshoot Connection`; it completes when the user runs the troubleshooter (a concrete, already-shipped action).
...
- **Out:** auto-open on first install, `globalState`, any shim change, screenshots/GIFs, telemetry, and any new setting.
