...
- **Context panel** — a sidebar view that follows whatever you're looking at: the errors and warnings in the current file, the branch of the repository that file belongs to, and the built-in briefs that fit right now, pre-filled with the file and line so you don't retype them. It reads only local sources — VS Code's own diagnostics and your local git — so nothing leaves your machine until you click a brief, and that click goes through the same pre-flight preview as everywhere else. This is the first slice: blame and related-index results for the line under your cursor come next.
