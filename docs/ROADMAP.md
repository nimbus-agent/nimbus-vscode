...
- The published (pinned) client types **ten** of them — all but decisions,
  ...
  the only thing this repo may program against.
...
**"Preview what leaves" pre-flight**, the **ambient context panel**, and
**Context-grounded Ask** have shipped — see **Already shipped** above; the
remaining Phase 2 item below is still open.
...
| **Typed `@`-mentions and Explorer drag-and-drop for Ask attachments** — autocomplete inside the composer, and dropping a file straight onto the panel, as a second and third way to attach alongside the Quick Pick and the *Attach to Ask* entries that shipped first | Attaching context is proven; this is what makes it fast. Deferred out of the first slice on purpose: `@`-completion means keyboard navigation and its own debounce inside a custom webview, the largest and least load-bearing part of the work, and drag-and-drop's edge cases (folders, files outside the workspace, untitled buffers, multi-select) each need an answer the chip vocabulary didn't yet have | `searchRanked` (mention completion only; drag-and-drop is local) | M |
